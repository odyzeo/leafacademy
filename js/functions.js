...
( function( $ ) {
	...
	$( function() {
		// Search toggle.
		/*$( '.search-toggle' ).on( 'click.leafacademy', function( event ) {
			var that    = $( this ),
    ...
    $(document).ready(function(){
       ...
       /* $('.block-front-news .slider .items').slick({
        ...
        function resizePostThumb(show, $postThumb){   
           if (!$postThumb) $postThumb =  $('.post-thumbnail');
           $postThumb.css({ height: 'auto' }); 
           $postThumb.each(function(){
               $(this).css({ height: $(this).height() });
           }); 
           if (show) $('.post-thumbnail-caption-wrapper-td', $postThumb).fadeTo(300,1);
        }
