...
function leafacademy_scripts() {
	...
	wp_enqueue_style( 'leafacademy-style', get_stylesheet_uri(),  array(), "6");
    ...
    wp_enqueue_script( 'leafacademy-script', get_template_directory_uri() . '/js/functions.js', array( 'jquery' ), '201511022', true );
}
...
function inline_featured_image( $atts, $content = null, $name = null ) {
    ...
    if( class_exists('Dynamic_Featured_Image') ) {
         ...
         if ( !isset( $attr['index'] ) ) { $attr['index'] = 1;    }
...
function leafacademy_metabox_team_member_meta_boxes() {
    ...
    add_meta_box(
        'leafacademy_metabox_team_member_type',
        __( "Member type", __TEXTDOMAIN__ ),
        'leafacademy_metabox_team_member_type_callback',
        'team_member',
        'normal',
        'low'
    );
...
function leafacademy_metabox_block() {
    ...
    add_meta_box(
        'leafacademy_metabox_block_tag',
        __( "Block tag", __TEXTDOMAIN__ ),
        'leafacademy_metabox_block_tag_callback',
        'grid_block',
        'normal',
        'low'
    );
    ...
    add_meta_box(
        'leafacademy_metabox_block_bgcolor',
        __( "Background color", __TEXTDOMAIN__ ),
        'leafacademy_metabox_block_bgcolor_callback',
        'grid_block',
        'normal',
        'low'
    );
    ...
    add_meta_box(
        'leafacademy_metabox_block_wide',
        __( "Block size", __TEXTDOMAIN__ ),
        'leafacademy_metabox_block_wide_callback',
        'grid_block',
        'normal',
        'low'
    );
    add_meta_box(
        'leafacademy_metabox_block_hide_title',
        __( "Block title options", __TEXTDOMAIN__ ),
        'leafacademy_metabox_block_hide_title_callback',
        'grid_block',
        'normal',
        'low'
    );
...
function leafacademy_metabox_block_tag_save_date( $post_id ) {
    ...
    // Check if our nonce is set.
    if ( ! isset( $_POST['leafacademy_metabox_block_tag_nonce'] ) 
         ||  ! isset( $_POST['leafacademy_metabox_block_bgcolor_nonce'] ) 
         ||  ! isset( $_POST['leafacademy_metabox_block_hide_title_nonce'] )
         ||  ! isset( $_POST['leafacademy_metabox_block_wide_nonce'] )
         ) {
        return;
    }
        ...
        foreach ( $posts as $post ) {
            ...
            //slideshow
             ...
             if( class_exists('Dynamic_Featured_Image') ) {
                ...
                // $html .= print_r($featured_images,true);
                 //return $html;
                 /*
                 if ( !isset( $attr['index'] ) ) { $attr['index'] = 1;    }
                 $fi = $featured_images[$attr['index']-1];
           ...
           // $content = wp_trim_words( $content, 30, '...');
            //  
            if (!empty($slides)){
                 $html .=  '<article class="item image '.$sizeClass.'"><div class="images-wrap">'.$slides.'</div></article>';
                 $blocksClass = 'block-gallery';
            }else if (trim(strip_tags($content))=="" && !empty($img)){
                if ($matchHeight){
                    $html .='<article class="item image do-bg-image '.$sizeClass.'"><img src="'.$img_url.'"></article>';
                }else{
                    $html .='<article class="item image '.$sizeClass.'"><img src="'.$img_url.'"></article>';
                }
            }else if (stripos($content, '<iframe')!==false && $meta_hidetitle){
                $html .='<article class="item image '.$sizeClass.'">'.$content.'</article>';
            }else{
                $content = wpautop( $content );
                $html .= '<article class="item bg-'.$meta_bgcolor.' '.$sizeClass.'">';
                if (!$meta_hidetitle) $html .= '<h2 class="title with-spacer">'.$post->post_title.'</h2>';
...
function leafacademy_metabox_post_meta_boxes() {
    ...
    add_meta_box(
        'leafacademy_metabox_post_link',
        __( "Button URL", __TEXTDOMAIN__ ),
        'leafacademy_metabox_post_link_callback',
        'post',
        'normal',
        'low'
    );
    add_meta_box(
        'leafacademy_metabox_post_link_label',
        __( "Button label", __TEXTDOMAIN__ ),
        'leafacademy_metabox_post_link_label_callback',
        'post',
        'normal',
        'low'
    ); 
    add_meta_box(
        'leafacademy_metabox_post_link_target',
        __( "Button options", __TEXTDOMAIN__ ),
        'leafacademy_metabox_post_link_target_callback',
        'post',
        'normal',
        'low'
    );
    ...
    array_map(function ($postType) {
        add_meta_box(
            'leafacademy_metabox_post_feed',
            __( "Show in news feed", __TEXTDOMAIN__ ),
            'leafacademy_metabox_post_feed_callback',
            $postType,
            'normal',
            'low'
        );
